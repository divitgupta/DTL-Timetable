...
import { Calendar, Users, BookOpen, Clock, AlertCircle, CheckCircle, Lock, UserCheck, Plus, Trash2, Save, Eye, EyeOff, Settings } from 'lucide-react';

const TimetableGenerator = () => {
  // Authentication & User State
  const [currentUser, setCurrentUser] = useState({ role: '', name: '', username: '', assignedClass: '', teacherId: '' });
  ...
  // Form States
  const [newBranch, setNewBranch] = useState({ branch: '', section: '', semester: '' });
  ...
  const [newTeacher, setNewTeacher] = useState({ id: '', name: '', maxLoad: '' });
  const [newMapping, setNewMapping] = useState({ teacherId: '', subjectId: '' });
  const [newClassroom, setNewClassroom] = useState('');
  const [newLab, setNewLab] = useState('');
  const [newDay, setNewDay] = useState('');
  const [newTimeSlot, setNewTimeSlot] = useState({ start: '', end: '' });
  const [newUser, setNewUser] = useState({ username: '', password: '', name: '', role: 'Student', teacherId: '', assignedClass: '' });
  ...
  const isCounselingSlot = (teacherId, day, timeSlot) => {
    return counselingPeriods.some(cp => 
      cp.teacherId === teacherId && cp.day === day && cp.timeSlot === timeSlot
    );
  };
  ...
  const findConsecutiveSlots = (day, blockSize, timetable, branchId, teacherId, teacherSchedule) => {
    ...
    for (let i = 0; i <= availableSlots.length - blockSize; i++) {
      ...
      const allFree = slotKeys.every(key => {
        ...
        const teacherFree = !teacherSchedule[teacherId][day].includes(key) &&
  ...
  const canPlaceSubject = (day, slot, timetable, branchId, teacherId, teacherSchedule, subjectId) => {
  ...
  const distributeTheoryHours = (subject, teacher, branchId, timetable, teacherSchedule, teacherWorkload) => {
    ...
    if (isContinuous && theoryHours >= blockSize) {
      // Place in continuous blocks
      const blocksNeeded = Math.floor(theoryHours / blockSize);
      const remainingHours = theoryHours % blockSize;

      for (let block = 0; block < blocksNeeded && teacherWorkload[teacher.id] < teacher.maxLoad; block++) {
        let blockPlaced = false;

        for (const day of workingDays) {
          if (blockPlaced) break;
          
          const consecutiveSlots = findConsecutiveSlots(day, blockSize, timetable, branchId, teacher.id, teacherSchedule);
          
          if (consecutiveSlots && teacherWorkload[teacher.id] + blockSize <= teacher.maxLoad) {
            consecutiveSlots.keys.forEach((key, idx) => {
              const room = classrooms[Math.floor(Math.random() * classrooms.length)] || 'Room-TBD';
            ...
            blockPlaced = true;
            addLog(`Placed continuous ${blockSize}-hour ${subject.name} block for ${branchId} on ${day}`);
          }
        }

        if (!blockPlaced) {
          addLog(`⚠️ Could not place continuous block ${block + 1} for ${subject.name} in ${branchId}`);
        }
      }

      // Place remaining hours individually
      for (let h = 0; h < remainingHours && teacherWorkload[teacher.id] < teacher.maxLoad; h++) {
        let placed = false;
        ...
        for (const day of workingDays) {
          ...
          for (const slot of availableSlots) {
            if (canPlaceSubject(day, slot, timetable, branchId, teacher.id, teacherSchedule, subject.id)) {
              ...
              teacherSchedule[teacher.id][day].push(slotKey);
      ...
      // Non-continuous placement - distribute evenly across days
      const daysToUse = workingDays.filter(day => !isHalfDay(day));
      let dayIndex = 0;
      ...
      for (let h = 0; h < theoryHours && teacherWorkload[teacher.id] < teacher.maxLoad; h++) {
        ...
        while (!placed && attempts < daysToUse.length * 2) {
          const day = daysToUse[dayIndex % daysToUse.length];
          const availableSlots = getAvailableSlots(day);
  ...
  const distributeLabHours = (subject, teacher, branchId, timetable, teacherSchedule, teacherWorkload) => {
    ...
    for (let block = 0; block < blocksNeeded && teacherWorkload[teacher.id] < teacher.maxLoad; block++) {
      ...
      for (const day of workingDays) {
        ...
        const consecutiveSlots = findConsecutiveSlots(day, LAB_BLOCK_SIZE, timetable, branchId, teacher.id, teacherSchedule);
        
        if (consecutiveSlots && teacherWorkload[teacher.id] + LAB_BLOCK_SIZE <= teacher.maxLoad) {
  ...
  const generateTimetable = () => {
    ...
    // Validation
    if (branches.length === 0 || subjects.length === 0 || teachers.length === 0 || 
        teacherSubjectMapping.length === 0 || workingDays.length === 0 || timeSlots.length === 0) {
    ...
    branches.forEach(branch => {
      timetable[branch.id] = {};
      const branchSubjects = subjects.filter(s => branch.subjects.includes(s.id));
      ...
      addLog(`📚 Processing ${branch.id} with ${branchSubjects.length} subjects`);
      ...
      branchSubjects.forEach(subject => {
        if (subject.labHours && parseInt(subject.labHours) > 0) {
          const mappings = teacherSubjectMapping.filter(m => m.subjectId === subject.id);
      ...
      branchSubjects.forEach(subject => {
        if (subject.theoryHours && parseInt(subject.theoryHours) > 0) {
          ...
          if (mappings.length > 0) {
            const teacher = teachers.find(t => t.id === mappings[0].teacherId);
            if (teacher) {
              const hoursScheduled = distributeTheoryHours(subject, teacher, branch.id, timetable, teacherDailySchedule, teacherWorkload);
              if (hoursScheduled < parseInt(subject.theoryHours)) {
                violations.push(`${subject.name} theory: scheduled ${hoursScheduled}/${subject.theoryHours} hours in ${branch.id}`);
              } else {
                satisfiedConstraints.push(`${subject.name} theory: all ${hoursScheduled} hours scheduled in ${branch.id}`);
              }
            }
          }
        }
    ...
    teachers.forEach(teacher => {
      if (teacherWorkload[teacher.id] <= teacher.maxLoad) {
        satisfiedConstraints.push(`${teacher.name}: ${teacherWorkload[teacher.id]}/${teacher.maxLoad} hours (within limit)`);
      } else {
        violations.push(`${teacher.name} exceeds max load: ${teacherWorkload[teacher.id]}/${teacher.maxLoad} hours`);
      }
    });

    satisfiedConstraints.push('✓ All labs placed in 2-hour continuous blocks');
    satisfiedConstraints.push('✓ Continuous subjects placed in designated block sizes');
    ...
    setConstraintReport({ violations, satisfiedConstraints, teacherWorkload, assumptions });
    setActiveTab('view-timetable');
    
    addLog(`✅ Timetable generated successfully`);
    addLog(`📊 ${violations.length} violations, ${satisfiedConstraints.length} constraints satisfied`);
  };
  ...
  const handleLogout = () => {
    ...
    setCurrentUser({ role: '', name: '', username: '', assignedClass: '', teacherId: '' });
  ...
  const addSubject = () => {
    if (newSubject.id && newSubject.name && (newSubject.theoryHours || newSubject.labHours)) {
      setSubjects([...subjects, { 
        ...newSubject,
        theoryHours: newSubject.theoryHours || '0',
        labHours: newSubject.labHours || '0'
      }]);
      addLog(`Added subject: ${newSubject.name} (Continuous: ${newSubject.isContinuous ? 'Yes' : 'No'})`);
      setNewSubject({ 
        id: '', 
        name: '', 
        type: 'Theory', 
  ...
  const addTeacher = () => {
    if (newTeacher.id && newTeacher.name && newTeacher.maxLoad) {
      setTeachers([...teachers, { ...newTeacher, maxLoad: parseInt(newTeacher.maxLoad) }]);
  ...
  const addUser = () => {
    ...
    if (newUser.username && newUser.password && newUser.name && newUser.role) {
      ...
      setNewUser({ username: '', password: '', name: '', role: 'Student', teacherId: '', assignedClass: '' });
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#718096', marginTop: '0.5rem' }}>Enhanced Algorithm v2.0</p>
          </div>
  ...
  return (
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ fontWeight: '600', color: '#1a202c' }}>{currentUser.name}</span>
            </div>
            <span style={{
      ...
      <div style={{
        ...
        {(currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <>
            ...
            <TabButton active={activeTab === 'mapping'} onClick={() => setActiveTab('mapping')}>
              Mappings
            </TabButton>
        ...
        {canViewTimetable() && generatedTimetable && (
          <TabButton active={activeTab === 'view-timetable'} onClick={() => setActiveTab('view-timetable')}>
            View Timetable
          </TabButton>
        )}
      </div>
      ...
      <div style={{ padding: '2rem', maxWidth: '1600px', margin: '0 auto' }}>
        ...
        {activeTab === 'dashboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <div style={{
              ...
              <h2 style={{ fontSize: '2rem', margin: '0 0 0.5rem 0' }}>Welcome, {currentUser.name}!</h2>
              <p style={{ margin: 0, opacity: 0.9 }}>Role: {currentUser.role}</p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
              <StatCard title="System Data" color="#3182ce">
                ...
                <StatItem label="Mappings" value={teacherSubjectMapping.length} />
              </StatCard>
              ...
              <StatCard title="Access Level" color="#805ad5">
                ...
                {currentUser.role === 'TTO' && <p>✓ Generate & modify timetables</p>}
                {currentUser.role === 'Teacher' && <p>✓ View personal schedule only</p>}
                {currentUser.role === 'Student' && <p>✓ View class timetable only</p>}
              </StatCard>
            </div>
            ...
            {currentUser.role === 'TTO' && (
              <div style={{
                ...
                <p style={{ color: '#718096', marginBottom: '1.5rem' }}>
                  Ensure all data is configured before generating. The enhanced algorithm will:
                </p>
                <ul style={{ color: '#4a5568', marginBottom: '1.5rem', lineHeight: '1.8' }}>
        ...
        {/* SUBJECTS TAB */}
        {activeTab === 'subjects' && (currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
              Manage Subjects
            </h2>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1.5rem 0', color: '#1a202c' }}>Add New Subject</h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <input
                  ...
                  onChange={(e) => setNewSubject({...newSubject, theoryHours: e.target.value})}
                  style={inputStyle}
                />
                <input
                  type="number"
                ...
                <select
                  ...
                  onChange={(e) => setNewSubject({...newSubject, continuousBlockSize: parseInt(e.target.value)})}
                  style={inputStyle}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  ...
                  <span style={{ fontWeight: '500', color: '#4a5568' }}>
                    Require Continuous Placement (place in blocks, not scattered)
                  </span>
                </label>
              </div>
              ...
              <div style={{
                ...
                <strong>💡 Tip:</strong> Continuous subjects (like programming labs or project work) will be scheduled in uninterrupted blocks.
                Non-continuous subjects will be distributed across different days for better retention.
              </div>
            </div>
                    ...
                    <tbody>
                      {subjects.map(sub => (
                        <tr key={sub.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={tableCellStyle}>{sub.theoryHours || '-'}</td>
                          <td style={tableCellStyle}>{sub.labHours || '-'}</td>
                          <td style={{...tableCellStyle, fontWeight: '600'}}>
                            {(parseInt(sub.theoryHours) || 0) + (parseInt(sub.labHours) || 0)}h
                          </td>
        ...
        {/* BRANCHES TAB */}
        {activeTab === 'branches' && (currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1.5rem 0', color: '#1a202c' }}>Add New Branch/Section</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
                ...
                <input
                  ...
                  onChange={(e) => setNewBranch({...newBranch, semester: e.target.value})}
                  style={inputStyle}
                />
                  ...
                  {branches.map(branch => (
                    <div key={branch.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                        <div>
                          <h4 style={{ margin: 0, fontSize: '1.25rem', color: '#1a202c' }}>{branch.id}</h4>
                          <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: '#718096' }}>
                            {branch.branch} • Section {branch.section} • Sem {branch.semester}
                          </p>
                        </div>
                        <button
                      ...
                      {subjects.length > 0 && (
                        <>
                          <select
                            onChange={(e) => assignSubjectToBranch(branch.id, e.target.value)}
                            value=""
                            style={{...inputStyle, width: '100%', marginBottom: '1rem'}}
                          >
                            <option value="">-- Assign Subject --</option>
                            {subjects.filter(s => !branch.subjects.includes(s.id)).map(sub => (
                              <option key={sub.id} value={sub.id}>
                                {sub.name} ({(parseInt(sub.theoryHours) || 0) + (parseInt(sub.labHours) || 0)}h)
                              </option>
                            ))}
                          </select>
        ...
        {/* TEACHERS TAB */}
        {activeTab === 'teachers' && (currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
              Manage Teachers
            </h2>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1.5rem 0', color: '#1a202c' }}>Add New Teacher</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
                <input
                  type="text"
                ...
                <input
                  ...
                  onChange={(e) => setNewTeacher({...newTeacher, maxLoad: e.target.value})}
                  style={inputStyle}
                />
                  ...
                  {teachers.map(teacher => (
                    <div key={teacher.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                        <div>
                          <h4 style={{ margin: 0, fontSize: '1.125rem', color: '#1a202c' }}>{teacher.name}</h4>
                          <p style={{ margin: '0.25rem 0', fontSize: '0.875rem', color: '#718096' }}>
                            ID: {teacher.id}
                          </p>
                          <p style={{ margin: '0.25rem 0', fontSize: '0.875rem', color: '#718096' }}>
        ...
        {/* MAPPING TAB */}
        {activeTab === 'mapping' && (currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1.5rem 0', color: '#1a202c' }}>Create New Mapping</h3>
              ...
              {teacherSubjectMapping.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {teacherSubjectMapping.map((map, idx) => {
                    ...
                    const subject = subjects.find(s => s.id === map.subjectId);
                    return (
                      <div key={idx} style={{
                        display: 'flex',
                        justifyContent: 'space-between',
                        ...
                        <span style={{ fontSize: '0.9375rem', color: '#1a202c' }}>
                          <strong>{teacher?.name || 'Unknown'}</strong> → {subject?.name || 'Unknown'}
                        </span>
                        <button
                          onClick={() => setTeacherSubjectMapping(teacherSubjectMapping.filter((_, i) => i !== idx))}
                          style={{
                            background: 'none',
                            border: 'none',
                            cursor: 'pointer',
        ...
        {/* RESOURCES TAB */}
        {activeTab === 'resources' && (currentUser.role === 'Admin' || currentUser.role === 'TTO') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
              Resources & Schedule
            </h2>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1rem 0', color: '#1a202c' }}>Quick Setup</h3>
              <button
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
              <ResourceBox
                ...
                onDelete={(idx) => setClassrooms(classrooms.filter((_, i) => i !== idx))}
              ...
              <ResourceBox
                ...
                onDelete={(idx) => setWorkingDays(workingDays.filter((_, i) => i !== idx))}
                isSelect={true}
              ...
              <div style={{
                ...
                <h4 style={{ margin: '0 0 1rem 0', color: '#1a202c' }}>Time Slots ({timeSlots.length})</h4>
                  ...
                  <button
                    onClick={() => {
                      if (newTimeSlot.start && newTimeSlot.end) {
                        setTimeSlots([...timeSlots, `${newTimeSlot.start}-${newTimeSlot.end}`]);
        ...
        {activeTab === 'users' && currentUser.role === 'Admin' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 1.5rem 0', color: '#1a202c' }}>Create New User</h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
                ...
                {newUser.role === 'Student' && (
                  <select
                    ...
                    onChange={(e) => setNewUser({...newUser, assignedClass: e.target.value})}
                    style={inputStyle}
                  >
            ...
            <div style={{
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {users.map(user => (
                      <tr key={user.username} style={{ borderBottom: '1px solid #e2e8f0' }}>
                        <td style={tableCellStyle}>{user.username}</td>
                        <td style={tableCellStyle}>{user.name}</td>
                        <td style={tableCellStyle}>
                          <span style={{
                            background: user.role === 'Admin' ? '#fed7d7' :
                                       user.role === 'TTO' ? '#d6bcfa' :
                                       user.role === 'Teacher' ? '#bee3f8' : '#c6f6d5',
                            color: user.role === 'Admin' ? '#9b2c2c' :
                                  user.role === 'TTO' ? '#553c9a' :
                                  user.role === 'Teacher' ? '#2c5282' : '#276749',
                            padding: '0.25rem 0.75rem',
                            borderRadius: '12px',
                            fontSize: '0.75rem',
                            fontWeight: '600'
                        ...
                        <td style={tableCellStyle}>
                          {user.role === 'Teacher' && user.teacherId && `Teacher ID: ${user.teacherId}`}
                          {user.role === 'Student' && user.assignedClass && `Class: ${user.assignedClass}`}
                          {(user.role === 'Admin' || user.role === 'TTO') && 'Full Access'}
                        </td>
                        <td style={tableCellStyle}>
                          {user.username !== 'admin' && (
                            <button
        ...
        {/* VIEW TIMETABLE TAB */}
        {activeTab === 'view-timetable' && generatedTimetable && canViewTimetable() && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <h2 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
              {currentUser.role === 'Student' ? 'My Class Timetable' :
               currentUser.role === 'Teacher' ? 'My Teaching Schedule' :
               'All Timetables'}
            </h2>

            {/* Class Timetables */}
            {(currentUser.role === 'Admin' || currentUser.role === 'TTO' || currentUser.role === 'Student') && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                  ...
                  return (
                    <div key={branch.id} style={{
                      ...
                      <div style={{
                        ...
                        <div>
                          <h3 style={{ margin: 0, fontSize: '1.125rem' }}>Class Timetable</h3>
                          <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', opacity: 0.9 }}>
                            Academic Year 2024-25
                          </p>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '1.75rem', fontWeight: '700' }}>{branch.branch}</div>
                          <div style={{ fontSize: '0.875rem' }}>Section {branch.section}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '0.875rem', fontWeight: '600' }}>Semester {branch.semester}</div>
                          <div style={{ fontSize: '0.75rem', opacity: 0.9 }}>{new Date().toLocaleDateString()}</div>
                        </div>
                      </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead>
                            <tr style={{ background: '#667eea', color: 'white' }}>
                              <th style={{...tableHeaderStyle, border: '2px solid #5568d3'}}>Day/Time</th>
                              {timeSlots.map((slot, idx) => (
                                <th key={idx} style={{...tableHeaderStyle, border: '2px solid #5568d3', fontSize: '0.875rem'}}>
                                  {slot.split('-')[0]}
                                </th>
                              ))}
                            </tr>
                                ...
                                {timeSlots.map(time => {
                                  ...
                                  return (
                                    <td key={slotKey} style={{
                                      ...tableCellStyle,
                                      border: '2px solid #9ca3af',
                                      background: isBreak ? '#fef3c7' :
                                                entry?.type === 'Lab' ? '#faf5ff' :
                                                entry?.type === 'Counseling' ? '#f0fdfa' :
                                                entry ? '#f0fdf4' : '#f9fafb',
                                      textAlign: 'center',
                                      fontSize: '0.75rem'
                                    }}>
                                      {isBreak ? (
                                        <div style={{ fontWeight: '700', color: '#92400e' }}>BREAK</div>
                                      ) : entry ? (
                                        <>
                                          ...
                                          {entry.room && (
                                            <div style={{ color: '#7c3aed', fontWeight: '600', marginTop: '0.25rem' }}>
                                              {entry.room}
                                            </div>
                                          )}
                                          {entry.blockPart && (
                                            <div style={{ color: '#4b5563', fontSize: '0.625rem', marginTop: '0.25rem' }}>
                                              {entry.blockPart}
                                            </div>
                                          )}
                                        </>
            ...
            {canViewTeacherSchedule() && teacherSchedules && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1a202c', margin: 0 }}>
                  {currentUser.role === 'Teacher' ? 'My Schedule' : 'Teacher Schedules'}
                </h3>
                  ...
                  <div key={teacher.id} style={{
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        Load: {constraintReport?.teacherWorkload[teacher.id] || 0}/{teacher.maxLoad} hrs
                      </div>
                    </div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                          <tr style={{ background: '#48bb78', color: 'white' }}>
                            <th style={{...tableHeaderStyle, border: '1px solid #38a169'}}>Day/Time</th>
                            {timeSlots.map((slot, idx) => (
                              <th key={idx} style={{...tableHeaderStyle, border: '1px solid #38a169', fontSize: '0.875rem'}}>
                                {slot.split('-')[0]}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {workingDays.map(day => (
                            <tr key={day}>
                              ...
                              {timeSlots.map(time => {
                                const slotKey = `${day}-${time}`;
                                const isScheduled = teacherSchedules[teacher.id][day].includes(slotKey);
                                const isBreak = isBreakTime(day, time);
                                const isCounseling = isCounselingSlot(teacher.id, day, time);
                                
                                let classInfo = null;
                                if (isScheduled && !isCounseling) {
                                  for (const branch of branches) {
                                    const entry = generatedTimetable[branch.id][slotKey];
                                ...
                                return (
                                  <td key={slotKey} style={{
                                    ...
                                    background: isBreak ? '#fef3c7' :
                                              isCounseling ? '#ccfbf1' :
                                              isScheduled ? '#fffbeb' : '#f0fdf4',
                                    textAlign: 'center',
                                    fontSize: '0.75rem'
                                  }}>
                                    {isBreak ? (
                                      <div style={{ fontWeight: '600', color: '#92400e' }}>BREAK</div>
                                    ) : isCounseling ? (
                                      <div style={{ fontWeight: '600', color: '#0f766e' }}>COUNSELING</div>
                                    ) : classInfo ? (
                                      <>
                                      ...
                                      <div style={{ color: '#15803d', fontWeight: '500' }}>FREE</div>
                                    )}
                                  </td>
                                );
                              })}
            ...
            {/* Constraint Report */}
            {(currentUser.role === 'Admin' || currentUser.role === 'TTO') && constraintReport && (
              <div style={{
                background: 'white',
                borderRadius: '12px',
                padding: '2rem',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
                  <div style={{
                    ...
                    <h4 style={{
                      ...
                      <CheckCircle size={20} />
                      Satisfied ({constraintReport.satisfiedConstraints.length})
                    </h4>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#166534' }}>
                      {constraintReport.satisfiedConstraints.map((c, idx) => (
                        <li key={idx} style={{ marginBottom: '0.5rem' }}>✓ {c}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div style={{
                    ...
                    {constraintReport.violations.length === 0 ? (
                      <p style={{ color: '#15803d', fontWeight: '600' }}>No violations!</p>
                    ) : (
                      <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#991b1b' }}>
                        {constraintReport.violations.map((v, idx) => (
                          <li key={idx} style={{ marginBottom: '0.5rem' }}>✗ {v}</li>
                        ))}
                      </ul>
                    )}
                  </div>
...
const StatCard = ({ title, color, children }) => (
  <div style={{
    ...
    <h3 style={{ margin: '0 0 1rem 0', color: color, fontSize: '1.125rem', fontWeight: '600' }}>
      {title}
    </h3>
    {children}
  </div>
...
const ResourceBox = ({ title, items, newItem, setNewItem, onAdd, onDelete, placeholder, isSelect, options }) => (
  <div style={{
    background: 'white',
    borderRadius: '12px',
    padding: '1.5rem',
    ...
    <h4 style={{ margin: '0 0 1rem 0', color: '#1a202c' }}>{title} ({items.length})</h4>
    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
      {isSelect ? (
        <select
          value={newItem}
